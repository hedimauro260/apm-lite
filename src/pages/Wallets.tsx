...
import { WalletSummaryCard } from "../components/ui/WalletSummaryCard"; // ✅ Novo import
...
export default function Wallets() {
  ...
  const isLoading =
    walletsLoading || transactionsLoading || assetsLoading || summaryLoading;
  ...
  const balanceTrend =
    balanceVariation > 0 ? "up" : balanceVariation < 0 ? "down" : "neutral";
  const inflowsTrend =
    inflowsVariation > 0 ? "up" : inflowsVariation < 0 ? "down" : "neutral";
  const outflowsTrend =
    outflowsVariation > 0 ? "up" : outflowsVariation < 0 ? "down" : "neutral";
  ...
  const [quickActionType, setQuickActionType] = useState<
    "deposit" | "withdraw" | "transfer" | "adjust" | undefined
  >();
  const [quickActionWalletId, setQuickActionWalletId] = useState<
    string | undefined
  >();
