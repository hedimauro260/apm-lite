...
export function LiveCryptoPrices() {
    ...
    return (
        <div className="w-full bg-surface border-t border-border overflow-hidden">
            {/* Header interno */}
            <div className="px-6 py-3 border-b border-border">
                <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-text-primary">Live Crypto Prices</span>
            ...
            <div className="relative w-full py-4 overflow-hidden flex bg-surface-elevated">
                ...
                <div className="flex gap-8 shrink-0 animate-[marquee_25s_linear_infinite] hover:[animation-play-state:paused]">
                    {marqueeItems.map((crypto, index) => {
                        const isPositive = crypto.price_change_percentage_24h >= 0;
                        return (
                            <div
                                key={`${crypto.id}-${index}`}
                                className="flex items-center gap-2 text-sm border-r border-border/50 pr-8 font-mono"
                            >
